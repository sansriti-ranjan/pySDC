...
def get_random_float():
    ...
    return np.random.uniform(
        low=-1.797693134862315e307, high=1.797693134862315e307, size=1
    )[0]
...
def test_float_conversion():
    ...
    for i in range(num_tests):
        ...
        assert np.isclose(
            res, rand
        ), f"Conversion between bytes and float failed for {rand}: result: {res}"
...
{1./res:.2e}"
    ...
    if nan_counter > 0:
        print(
            f"When flipping bits, we got nan {nan_counter} times out of {num_tests} tests"
        )
...
def test_fault_stats(numprocs):
    ...
    for strategy in stats.strategies:
        ...
        recovered_mask = stats.get_mask(
            strategy=strategy, key="recovered", op="eq", val=True
        )
        index = stats.get_index(mask=fixable_mask)

        assert all(
            fixable_mask[:-1] == [False, True, False]
        ), "Error in generating mask of fixable faults"
        assert all(index == [1, 3]), "Error when converting to  index"

        combinations = np.array(
            stats.get_combination_counts(dat, keys=["bit"], mask=fixable_mask)
        )
        assert all(combinations == [1.0, 1.0]), "Error when counting combinations"

        recovered = len(dat["recovered"][recovered_mask])
        crashed = len(
            dat["error"][dat["error"] == np.inf]
        )  # on some systems the last run crashes...
        assert (
