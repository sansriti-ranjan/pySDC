...
def run_Lorenz(efficient, skip_residual_computation, num_procs=1):
    ...
    from pySDC.projects.Resilience.sweepers import (
        generic_implicit_efficient,
        generic_implicit,
    )
    ...
    sweeper_params["skip_residual_computation"] = (
        ("IT_CHECK", "IT_FINE", "IT_COARSE", "IT_DOWN", "IT_UP")
    ...
    description["sweeper_class"] = (
        generic_implicit_efficient if efficient else generic_implicit
    )
...
def run_Schroedinger(efficient=False, num_procs=1, skip_residual_computation=False):
    ...
    sweeper_params["skip_residual_computation"] = (
        ("IT_FINE", "IT_COARSE", "IT_DOWN", "IT_UP")
        if skip_residual_computation
        else ()
    )
    ...
    description["sweeper_class"] = (
        imex_1st_order_efficient if efficient else imex_1st_order
    )
    description["sweeper_params"] = sweeper_params
    description["level_params"] = level_params
    description["step_params"] = step_params
...
def test_generic_implicit_efficient(skip_residual_computation=True):
    stats_normal = run_Lorenz(
        efficient=False, skip_residual_computation=skip_residual_computation
    )
    stats_efficient = run_Lorenz(
        efficient=True, skip_residual_computation=skip_residual_computation
    )
...
def test_residual_skipping():
    ...
    assert_sameness(
        stats_normal, stats_efficient, "generic_implicit", check_residual=False
    )
...
def test_residual_skipping_with_residual_tolerance():
    ...
    assert_sameness(
        stats_normal, stats_efficient, "imex_first_order", check_residual=False
    )
...
def assert_sameness(stats_normal, stats_efficient, sweeper_name, check_residual=True):
    ...
    for me in get_list_of_types(stats_normal):
        ...
        if (
            "timing" in me
            or all(you is None for you in normal)
            or (not check_residual and "residual" in me)
        ):
            continue
        assert np.allclose(
