...
def test_convergence_by_iter(maxiter):
    ...
    assert (
        niter == maxiter
    ), f"Wrong number of iterations! Expected {maxiter}, but got {niter}!"
...
def test_convergence_by_increment(e_tol):
    ...
    e_em = [
        ...
        for me in get_sorted(
            stats, type="error_embedded_estimate_post_iteration", sortby="iter"
        )
    ]
    ...
    assert (
        e_em_before_convergence > e_tol
    ), "Embedded error estimate was below threshold before convergence!"
    assert (
        e_em_at_convergence <= e_tol
    ), "Step terminated before convergence by increment was achieved!"
...
def test_convergence_by_residual(restol):
    ...
    res = [
        me[1] for me in get_sorted(stats, type="residual_post_iteration", sortby="iter")
    ]
    ...
    assert (
        res_before_convergence > restol
    ), "Residual was below threshold before convergence!"
    assert (
        res_at_convergence <= restol
    ), "Step terminated before convergence by residual was achieved!"
