...
class FaultStats:
    ...
    def run_stats_generation(
            ...
            if type(val) == int:
                ...
                for me in val:
                    ...
                    self.run_stats_generation(
                        runs=runs, step=step, comm=comm, kwargs_range=kwargs_range_me
                    )
                return None
        ...
        if reload:
            ...
            if comm.rank == 0:
                already_completed = np.array(
                    [
                        self.load(strategy=strategy, faults=True).get("runs", 0)
                        for strategy in self.strategies
                    ]
                )
                sorting_index_ = np.argsort(already_completed)
                sorting_index = sorting_index_[
                    already_completed[sorting_index_] < max_runs
                ]
        ...
        for j in range(0, len(strategies), comm.size):
            for f in self.faults:
                ...
                self.generate_stats(
                    strategy=strategies[
                        j + (comm.rank % len(strategies) % (len(strategies)) - j)
                    ],
                    runs=runs_partial,
                    faults=f,
                    reload=reload,
        ...
        self.run_stats_generation(
            ...
            comm=comm,
            _reload=True,
            _runs_partial=_runs_partial + step,
        )

        return None

    def generate_stats(
        self, strategy=None, runs=1000, reload=True, faults=True, comm=None
    ):
        """
        ...
        if reload:
            ...
            if (
                ...
                and already_completed["runs"] <= runs
                and comm.rank == 0
            ):
                for k in dat.keys():
                    dat[k][
                        : min([already_completed["runs"], runs])
                    ] = already_completed.get(k, [])
        else:
            already_completed = {"runs": 0}
        ...
        for i in range(already_completed["runs"], runs):
            ...
            stats, controller, Tend = self.single_run(
                strategy=strategy, run=i, faults=faults, space_comm=space_comm
            )
            ...
            total_newton_iteration = sum(
                [k[1] for k in get_sorted(stats, type="work_newton")]
            )
            ...
            if faults:
                if len(faults_run) > 0:
                    ...
                    assert (
                        self.mode == "regular"
                    ), f"No faults where recorded in run {i} of strategy {strategy.name}!"
            dat["error"][i] = error
            dat["total_iteration"][i] = total_iteration
            dat["total_newton_iteration"][i] = total_newton_iteration
            dat["restarts"][i] = sum(
                [me[1] for me in get_sorted(stats, type="restarts")]
            )
    ...
    def single_run(
        ...
        hook_class = (
            hook_collection
            + [LogWork]
            + ([LogData] if hook_class is None else hook_class)
        )
        ...
        if faults:
            ...
            if self.mode == "random":
                ...
                fault_stuff["rng"] = np.random.RandomState(run)
                fault_stuff["fault_frequency_iter"] = self.kwargs[
                    "fault_frequency_iter"
                ]
                ...
                raise NotImplementedError(
                    f"Don't know how to add faults in mode {self.mode}"
                )
    ...
    def compare_strategies(self, run=0, faults=False, ax=None):  # pragma: no cover
        ...
        k_ax = ax.twinx()
        ls = [
            "-." if type(strategy) == HotRodStrategy else "-"
            for strategy in self.strategies
        ]
        [
            self.scrutinize_visual(self.strategies[i], run, faults, ax, k_ax, ls[i])
            for i in range(len(self.strategies))
        ]

        # make a legend
        [
            k_ax.plot([None], [None], label=strategy.label, color=strategy.color)
            for strategy in self.strategies
        ]
    ...
    def scrutinize_visual(
        ...
        ax.plot(
            [me[0] for me in e_loc],
            [me[1] for me in e_loc],
        ...
        if plot_restarts:
            ...
            [
                ax.axvline(me[0], color="black", ls="-.") if me[1] else ""
                for me in restarts
            ]
        ...
        if store:
            fig.tight_layout()
            plt.savefig(
                f"data/{self.get_name()}-{strategy.name}-details.pdf", transparent=True
            )
    ...
    def scrutinize(self, strategy, run, faults=True):
        ...
        stats, controller, Tend = self.single_run(
            strategy=strategy, run=run, faults=faults, force_params=force_params
        )
        ...
        if t < Tend:
            error = np.inf
            print(
                f"Final time was not reached! Code crashed at t={t:.2f} instead of reaching Tend={Tend:.2f}"
            )
        else:
            error = self.get_error(u, t, controller, strategy)
...
{error < recovery_thresh}"
        ...
        print(
            f"k: sum: {np.sum(k)}, min: {np.min(k)}, max: {np.max(k)}, mean: {np.mean(k):.2f},"
        )
        ...
        print(
            f"dt: min: {np.min(dt):.2e}, max: {np.max(dt):.2e}, mean: {np.mean(dt):.2e}"
        )
        ...
        for f in faults:
            print(
                f" {f[0]:6.2f} | {f[1][0]:5d} | {f[1][1]:4d} | {f[1][2]:4d} | {f[1][4]:3d} | {f[1][5]:4d} |",
                f[1][3],
            )
    ...
    def get_name(self, strategy=None, faults=True, mode=None):
        ...
        if mode == "regular":
            mode_thing = (
                f'-regular{self.kwargs["fault_frequency_iter"] if faults else ""}'
            )
        else:
            mode_thing = ""

        return (
            f"{prob_name}{strategy_name}{fault_name}-{self.num_procs}procs{mode_thing}"
        )
    ...
    def get_thresh(self, strategy=None):
        ...
        return (
            self.recovery_thresh_abs + self.recovery_thresh * fault_free["error"].mean()
        )
    ...
    def get_recovered(self, **kwargs):
        ...
        if "strategy" not in kwargs.keys():
            ...
            try:
                ...
                with_faults["recovered"] = with_faults["error"] < self.get_thresh(
                    kwargs["strategy"]
                )
    ...
    def plot_things_per_things(
        ...
        if store:
            fig.tight_layout()
            plt.savefig(
                f"data/{self.get_name()}-{thingA if name is None else name}_per_{thingB}.pdf",
                transparent=True,
            )
            plt.close(fig)
    ...
    def plot_recovery_thresholds(
        self, strategies=None, thresh_range=None, ax=None
    ):  # pragma: no cover
        """
        ...
        strategies = self.strategies if strategies is None else strategies
        thresh_range = (
            1 + np.linspace(-4e-2, 4e-2, 100) if thresh_range is None else thresh_range
        )
        if ax is None:
            fig, ax = plt.subplots(1, 1)
        ...
        for strategy_idx in range(len(strategies)):
            ...
            for thresh_idx in range(len(thresh_range)):
                rec_mask = (
                    with_faults["error"]
                    < thresh_range[thresh_idx] * fault_free["error"].mean()
                )
                rec_rates[strategy_idx][thresh_idx] = len(
                    with_faults["error"][rec_mask]
                ) / len(with_faults["error"])

            ax.plot(
                thresh_range,
                rec_rates[strategy_idx],
                color=strategy.color,
    ...
    def analyse_adaptivity_single(self, run):  # pragma: no cover
        ...
        for faults in [True, False]:
            s, c, _ = self.single_run(
                ...
                run=run,
                faults=faults,
                hook_class=hook_collection + [LogUAllIter],
            )
            stats += [s]
            controllers += [c]
        ...
        e_em = [
            [
                ...
                for me in get_sorted(
                    stat, type="error_embedded_estimate", time=t_fault, sortby="iter"
                )
            ]
            for stat in stats
        ]
        ...
        e_glob = [
            abs(
                u_end[i][1] - controllers[i].MS[0].levels[0].prob.u_exact(t=u_end[i][0])
            )
            for i in [0, 1]
        ]
...
{dat["problem_pos"][i]}'
    ...
    def get_mask(
        ...
        if compare_faults:
            if val is not None:
                raise ValueError(
                    "Can't use val and compare_faults in get_mask at the same time!"
                )
            else:
    ...
    def get_fixable_faults_only(self, strategy):
        ...
        fixable = strategy.get_fixable_params(
            maxiter=strategy.get_custom_description(self.prob, self.num_procs)[
                "step_params"
            ]["maxiter"]
        )
    ...
    def get_statistics_info(
        self, mask=None, strategy=None, print_all=False, ax=None
    ):  # pragma: no cover
        """
        ...
        # print a header
        print(
            f' tot: {len(dat["error"][mask]):6} | avg. counts | mean deviation | unique entries'
        )
        print("-------------------------------------------------------------")
    ...
    def combinations_histogram(
        self, dat=None, keys=None, mask=None, ax=None
    ):  # pragma: no cover
        """
    ...
    def get_combination_histogram(
        self, dat=None, keys=None, mask=None
    ):  # pragma: no cover
        """
        ...
        # load default values
        dat = (
            self.load(strategy=self.strategies[0], faults=True) if dat is None else dat
        )
        ...
        occurrences, bins = np.histogram(
            combination_counts, bins=np.arange(max(combination_counts) + 1)
        )
    ...
    def get_max_combinations(self, dat=None):
        ...
        """
        stats, controller, Tend = self.single_run(
            strategy=self.strategies[0], run=0, faults=True
        )
    ...
    def get_combination_counts(self, dat, keys, mask):
        ...
        for i in range(len(unique_vals)):
            inner_mask = self.get_mask(
                key=key, val=unique_vals[i], op="eq", old_mask=mask
            )
            if len(keys) > 1:
    ...
    def count_occurrences(self, vals):
        ...
        if len(counts) > 0:
            return (
                counts.mean(),
                sum(abs(counts - counts.mean())) / len(counts),
                len(counts),
            )
        else:
            return None, None, 0
    ...
    def bar_plot_thing(
        ...
        if store:
            fig.tight_layout()
            plt.savefig(
                f"data/{self.get_name()}-{thing if name is None else name}-barplot.pdf",
                transparent=True,
            )
            plt.close(fig)
    ...
    def fault_frequency_plot(
        self, ax, iter_ax, kwargs_range, strategy=None
    ):  # pragma: no cover
        func_args = locals()
        ...
        results["recovery_rate"] = [
            len(all_data[key]["recovered"][all_data[key]["recovered"] is True])
            / len(all_data[key]["recovered"])
            for key in all_data.keys()
        ]
        # results['iterations'] = [np.mean(all_data[key]['total_iteration']) for key in all_data.keys()]
        results["iterations"] = [
            np.mean(all_data[key]["total_iteration"][all_data[key]["error"] != np.inf])
            for key in all_data.keys()
        ]
...
def main():
    ...
    stats_analyser.fault_frequency_plot(
        ax=ax, iter_ax=iter_ax, kwargs_range=kwargs_range
    )
    ...
    stats_analyser.plot_recovery_thresholds(
        ax=ax, thresh_range=np.logspace(-1, 1, 1000)
    )
    ax.axvline(
        stats_analyser.get_thresh(BaseStrategy()),
        color="grey",
        ls=":",
