...
def get_results_from_stats(stats, var, val, hook_class=LogLocalErrorPostStep):
    ...
    if hook_class == LogLocalErrorPostStep:
        e_extrapolated = np.array(
            get_sorted(stats, type="error_extrapolation_estimate")
        )[:, 1]
...
def multiple_runs(
    ...
    for i in range(0, len(dt_list)):
        desc = {
            ...
            "convergence_controllers": {
                EstimateEmbeddedError.get_implementation(
                    flavor=embedded_error_flavor, useMPI=False
                ): {},
        ...
        e_local = abs(
            level.prob.u_exact(
                t=level.time + level.dt, u_init=level.u[0], t_init=level.time
            )
            - level.u[-1]
        )
...
def get_accuracy_order(results, key="e_embedded", thresh=1e-14, var="dt"):
    ...
    assert (
        var in results
    ), f"ERROR: expecting the list of {var} in the results dictionary"
    ...
    for i in range(1, len(dt_list)):
        ...
        try:
            tmp = np.log(results[key][i] / results[key][i - 1]) / np.log(
...
def plot_all_errors(
    ...
    ax.plot(
        [None, None], color="black", label=r"$\epsilon_\mathrm{extrapolated}$", ls=":"
    )
...
def check_order_with_adaptivity():
    ...
    for serial in [True, False]:
        ...
        if serial:
            fig.savefig(
                "data/error_estimate_order_adaptivity.png", dpi=300, bbox_inches="tight"
            )
        else:
            fig.savefig(
                "data/error_estimate_order_adaptivity_parallel.png",
                dpi=300,
                bbox_inches="tight",
            )
        plt.close(fig)
...
def check_order_against_step_size():
    ...
    for serial in [True, False]:
        ...
        if serial:
            ...
            fig.savefig(
                "data/error_estimate_order_parallel.png", dpi=300, bbox_inches="tight"
            )
        plt.close(fig)
