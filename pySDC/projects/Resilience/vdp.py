...
def plot_step_sizes(stats, ax, e_em_key="error_embedded_estimate"):
    ...
    t = np.array(
        [me[0] for me in get_sorted(stats, type="u", recomputed=False, sortby="time")]
    )

    e_em = np.array(get_sorted(stats, type=e_em_key, recomputed=False, sortby="time"))[
        :, 1
    ]
    dt = np.array(get_sorted(stats, type="dt", recomputed=False, sortby="time"))
    restart = np.array(
        get_sorted(stats, type="restart", recomputed=None, sortby="time")
    )
...
def plot_avoid_restarts(stats, ax, avoid_restarts):
    ...
    ax.plot(
        [me[0] for me in sweeps],
        np.cumsum([me[1] for me in sweeps]),
        color=color,
        label=f"{label} avoid_restarts",
    )
...
def fetch_test_data(stats, comm=None, use_MPI=False):
    ...
    for type in types:
        if type not in get_list_of_types(stats):
            raise ValueError(
                f'Can\'t read type "{type}" from stats, only got',
                get_list_of_types(stats),
            )

        if comm is None or use_MPI is False:
            data[type] = [
                me[1]
                for me in get_sorted(stats, type=type, recomputed=None, sortby="time")
            ]
        else:
            data[type] = [
                me[1]
                for me in get_sorted(
                    stats, type=type, recomputed=None, sortby="time", comm=comm
                )
            ]
    return data
...
def check_adaptivity_with_avoid_restarts(comm=None, size=1):
    ...
    for avoid_restarts in [True, False]:
        ...
        results["sweeps"][avoid_restarts] = sum(
            [
                me[1]
                for me in get_sorted(stats, type="sweeps", recomputed=None, comm=comm)
            ]
        )
        results["restarts"][avoid_restarts] = sum(
            [me[1] for me in get_sorted(stats, type="restart", comm=comm)]
        )

    fig.tight_layout()
    fig.savefig(
        f'data/vdp-{size}procs{"-use_MPI" if comm is not None else ""}-avoid_restarts.png'
    )
...
def check_step_size_limiter(size=4, comm=None):
    ...
    for limit_step_sizes in [False, True]:
        ...
        convergence_controller_classes = [
            type(me) for me in controller.convergence_controllers
        ]
        for c in convergence_controller_classes:
            assert (
                convergence_controller_classes.count(c) == 1
            ), f"Convergence controller {c} added multiple times"
        ...
        if not limit_step_sizes:
            ...
            dt_min = min(
                dt_numpy[size:-size]
            )  # The first and last step might fall below the limits
    ...
    if comm is None:
        print(
            f"Passed step size limiter test with {size} ranks in nonMPI implementation"
        )
    else:
        if comm.rank == 0:
            print(
                f"Passed step size limiter test with {size} ranks in MPI implementation"
            )
...
def interpolation_stuff():  # pragma: no cover
    ...
    for i in range(3):
        ...
        axs[2].plot(
            [me[0] for me in k], np.cumsum([me[1] for me in k]), color=colors[i]
        )
        restart_ax.plot(
            [me[0] for me in restarts],
            np.cumsum([me[1] for me in restarts]),
            color=colors[i],
            ls="--",
        )
