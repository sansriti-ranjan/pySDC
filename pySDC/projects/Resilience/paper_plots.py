...
def analyse_resilience(problem, path="data/stats", **kwargs):  # pragma: no cover
    ...
    not_overflow = stats_analyser.get_mask(
        strategy=strategy, key="bit", val=1, op="uneq", old_mask=not_fixed
    )
...
def plot_recovery_rate_recoverable_only(
    stats_analyser, fig, ax, **kwargs
):  # pragma: no cover
    """
    ...
    for i in range(len(stats_analyser.strategies)):
        fixable = stats_analyser.get_fixable_faults_only(
            strategy=stats_analyser.strategies[i]
        )
...
def compare_recovery_rate_problems():  # pragma: no cover
    ...
    fig, axs = plt.subplots(
        2, 2, figsize=figsize_by_journal(JOURNAL, 1, 0.8), sharey=True
    )
    [
        plot_recovery_rate_recoverable_only(
            stats[i], fig, axs.flatten()[i], ylabel="", title=titles[i]
        )
...
def plot_efficiency_polar_vdp(problem, path="data/stats"):  # pragma: no cover
    ...
    fig, ax = plt.subplots(
        ...
        figsize=figsize_by_journal(JOURNAL, 0.7, 0.5),
        layout="constrained",
    )
...
def plot_efficiency_polar_single(stats_analyser, ax):  # pragma: no cover
    ...
    # TODO: fix docs
    mask = (
        stats_analyser.get_mask()
    )  # get empty mask, potentially put in some other mask later
    ...
    for strategy in stats_analyser.strategies:
        ...
        detected = stats_analyser.get_mask(
            strategy=strategy, key="total_iteration", op="gt", val=iterations_no_faults
        )
        rec_mask = stats_analyser.get_mask(
            strategy=strategy, key="recovered", op="eq", val=True, old_mask=detected
        )
        if rec_mask.any():
            extra_iterations = (
                np.mean(dat["total_iteration"][rec_mask]) - iterations_no_faults
            )
        else:
            extra_iterations = 0
    ...
    for s in stats_analyser.strategies:
        ax.plot(
            theta,
            res_norm[s.name] + [res_norm[s.name][0]],
            label=s.label,
            color=s.color,
            marker=s.marker,
        )
...
def plot_adaptivity_stuff():  # pragma: no cover
    ...
    fig, axs = plt.subplots(
        3, 1, figsize=figsize_by_journal(JOURNAL, scale, 1), sharex=True, sharey=False
    )
    ...
    for strategy in [BaseStrategy, AdaptivityStrategy, IterateStrategy]:
        stats, _, _ = stats_analyser.single_run(
            ...
            hook_class=[LogLocalErrorPostStep, LogData],
        )
        ...
        if strategy == BaseStrategy:
            ...
            axs[0].plot(
                [me[0] for me in u], [me[1][0] for me in u], color="black", label=r"$u$"
            )
...
def plot_quench_solution():  # pragma: no cover
    ...
    stats, controller, _ = run_quench(
        custom_description=custom_description, Tend=strategy.get_Tend(run_quench)
    )
    ...
    ax.axhline(
        prob.u_thresh, label="$T_\mathrm{thresh}$", ls="--", color="grey", zorder=-1
    )
...
def plot_Lorenz_solution():  # pragma: no cover
    ...
    print(u[-1], e)
    ax.plot(
        [me[1][0] for me in u], [me[1][1] for me in u], [me[1][2] for me in u], ls="--"
    )
    ################
    ax.set_xlabel("x")
    ax.set_ylabel("y")
...
def work_precision():  # pragma: no cover
    ...
    # Quench stuff
    fig, axs = get_fig(
        x=3, y=1, figsize=figsize_by_journal("Springer_Numerical_Algorithms", 1, 0.47)
    )
    quench_params = {
        **all_params,
        "problem": run_quench,
    ...
    execute_configurations(
        **{**quench_params, "work_key": "k_SDC", "precision_key": "k_Newton"}, ax=axs[2]
    )
    execute_configurations(
        **{**quench_params, "work_key": "param", "precision_key": "restart"}, ax=axs[1]
    )
    execute_configurations(
        **{**quench_params, "work_key": "t", "precision_key": "e_global_rel"}, ax=axs[0]
    )
