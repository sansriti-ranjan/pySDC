...
def get_data(stats, recomputed=False):
    ...
    data = {
        ...
        "t": np.array(
            [me[0] for me in get_sorted(stats, type="u", recomputed=recomputed)]
        ),
        "dt": np.array(
            [me[1] for me in get_sorted(stats, type="dt", recomputed=recomputed)]
        ),
        "t_dt": np.array(
            [me[0] for me in get_sorted(stats, type="dt", recomputed=recomputed)]
        ),
        "e_em": np.array(
            get_sorted(stats, type="error_embedded_estimate", recomputed=recomputed)
        )[:, 1],
        "e_ex": np.array(
            get_sorted(
                stats, type="error_extrapolation_estimate", recomputed=recomputed
            )
        )[:, 1],
        "restarts": np.array(get_sorted(stats, type="restart", recomputed=None))[:, 1],
        "t_restarts": np.array(get_sorted(stats, type="restart", recomputed=None))[
            :, 0
        ],
        "sweeps": np.array(get_sorted(stats, type="sweeps", recomputed=None))[:, 1],
    }
    data["ready"] = np.logical_and(
        data["e_ex"] != np.array(None), data["e_em"] != np.array(None)
    )
...
def plot_error(data, ax, use_adaptivity=True, plot_restarts=False):
    ...
    e_ax.plot(
        data["t"][data["ready"]],
        data["e_ex"][data["ready"]],
        label=r"$\epsilon_\mathrm{extrapolated}$",
        ls="--",
    )
    e_ax.plot(
        data["t"][data["ready"]],
    ...
    if plot_restarts:
        [
            ax.axvline(t_restart, ls="-.", color="black", alpha=0.5)
            for t_restart in data["restart_times"]
        ]
...
def residual_adaptivity(plot=False):
    ...
    ), f"Max. allowed residual is {max_res:.2e}, but got {max_residual:.2e}!"
    dt_std = np.std([me[1] for me in dt])
    assert (
        dt_std != 0
    ), f"Expected the step size to change, but standard deviation is {dt_std:.2e}!"
...
def main():
    ...
    for use_adaptivity in [True, False]:
        ...
        for num_procs in [1, 4]:
            custom_description["convergence_controllers"][HotRod] = {
                "HotRod_tol": 1,
            ...
            if use_adaptivity:
                fig.savefig(
                    f"data/piline_hotrod_adaptive_{num_procs}procs.png",
                    bbox_inches="tight",
                    dpi=300,
                )
            else:
                fig.savefig(
                    f"data/piline_hotrod_{num_procs}procs.png",
                    bbox_inches="tight",
                    dpi=300,
                )
            if use_adaptivity and num_procs == 4:
                ...
                sol_fig.savefig(
                    "data/piline_solution_adaptive.png", bbox_inches="tight", dpi=300
                )
                plt.close(sol_fig)
