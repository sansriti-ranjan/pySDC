...
special_params = {
    "inexact": {
        EstimateEmbeddedErrorCollocation: {"adaptive_coll_params": coll_params_inexact}
    },
    "refinement": {
        EstimateEmbeddedErrorCollocation: {
            "adaptive_coll_params": coll_params_refinement
        }
    },
    "reduce": {
        EstimateEmbeddedErrorCollocation: {"adaptive_coll_params": coll_params_reduce}
    },
    "standard": {},
    "type": {
        EstimateEmbeddedErrorCollocation: {"adaptive_coll_params": coll_params_type}
    },
}
...
def compare_adaptive_collocation(prob):
    ...
    for i in range(len(special_params.keys())):
        ...
        custom_description = {
            **base_params,
            "convergence_controllers": special_params[key],
        }
...
def check_order(prob, coll_name, ax, k_ax):
    ...
    for i in range(len(dt_range)):
        ...
        custom_description = {
            **base_params,
            "convergence_controllers": special_params[coll_name],
            **new_params,
        }
        custom_controller_parameters = {"logger_level": 30}
        stats, _, _ = prob(
        ...
        sweeper_params = get_sorted(stats, type="sweeper_params", sortby="iter")
        converged_solution = [
            sweeper_params[i][1] != sweeper_params[i + 1][1]
            for i in range(len(sweeper_params) - 1)
        ] + [True]
        idx = np.arange(len(converged_solution))[converged_solution]
        labels = [
            sweeper_params[i][1][label_keys.get(coll_name, "num_nodes")] for i in idx
        ]
        e_loc = np.array(
            [
                me[1]
                for me in get_sorted(
                    stats, type="e_local_post_iteration", sortby="iter"
                )
            ]
        )[converged_solution]
        ...
        e_em_raw = [
            ...
            for me in get_sorted(
                stats,
                type="error_embedded_estimate_collocation_post_iteration",
                sortby="iter",
            )
        ]
        e_em = np.array(
            (e_em_raw + [None] if coll_name == "refinement" else [None] + e_em_raw)
        )
        coll_order = np.array(
            [me[1] for me in get_sorted(stats, type="coll_order", sortby="iter")]
        )[converged_solution]
    ...
    for i in range(num_sols):
        ...
        ax.loglog(
            result["dt"],
            result[i],
            label=f"{label} nodes: order: {np.mean(order):.1f}",
            color=CMAP[i],
        )
...
def adaptivity_collocation(plotting=False):
    ...
    convergence_controllers[AdaptivityCollocation] = {
        "adaptive_coll_params": adaptive_coll_params,
        "e_tol": e_tol,
    }
    ...
    stats, controller, _ = run_vdp(
        custom_description=description, custom_controller_params=controller_params
    )

    e_em = get_sorted(
        stats, type="error_embedded_estimate_collocation", recomputed=False
    )
    assert (
