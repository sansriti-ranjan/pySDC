...
def plot_stability(
    stats, ax=None, iter=None, colors=None, crosshair=True, fill=False, **kwargs
):
    """
    ...
    for i in iter:
        ...
        if fill:
            ax.contourf(
                ...
                Y,
                abs(U),
                levels=[-np.inf, 1 - np.finfo(float).eps],
                colors=colors[i - 1],
                alpha=0.5,
            )
...
def plot_contraction(
    stats, fig=None, ax=None, iter=None, plot_increase=False, cbar=True, **kwargs
):
    """
    ...
    assert (
        len(iter) > 1
    ), "Need to compute the contraction factor across multiple iterations!"
    ...
    if plot_increase:
        ...
        cs = ax.contourf(
            X, Y, np.where(rho_log <= 0, rho_log, None), levels=500, **kwargs
        )
...
def plot_increment(stats, fig=None, ax=None, iter=None, cbar=True, **kwargs):
    ...
    rho_log = np.log(
        np.reshape(
            rho_avg, (len(np.unique(lambdas.real)), len(np.unique(lambdas.imag)))
        )
    )
...
def compare_contraction():
    ...
    fig, axs = plt.subplots(
        1, 2, figsize=(12, 5.5), gridspec_kw={"width_ratios": [0.8, 1]}
    )
    precons = ["TRAP", "IE"]
    ...
    for i in range(len(precons)):
        ...
        stats, controller, Tend = run_dahlquist(
            custom_description=custom_description, res=(400, 400)
        )
