...
def multiple_runs(prob, dts, num_nodes, quad_type="RADAU-RIGHT"):
    ...
    for dt in dts:
        ...
        res[dt] = {}
        res[dt]["e_loc"] = max(
            [me[1] for me in get_sorted(stats, type="e_local_post_step")]
        )
        res[dt]["e_ex"] = max(
            [me[1] for me in get_sorted(stats, type="error_extrapolation_estimate")]
        )
...
def plot_and_compute_order(ax, res, num_nodes, coll_order):
    ...
    for key in keys:
        ...
        order = np.log(errors[mask][1:] / errors[mask][:-1]) / np.log(
            dts[mask][1:] / dts[mask][:-1]
        )
