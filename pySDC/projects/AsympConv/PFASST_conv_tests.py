...
def run_diffusion(nsweeps):
    ...
    for i in range(-3, 10):
        ...
        if nsweeps == 3 and (i == -3 or i == 9):
            assert np.mean(niters) <= 2, (
                "ERROR: too much iterations for diffusive asymptotics, got %s"
                % np.mean(niters)
            )
...
def run_advection(nsweeps):
    ...
    for i in range(-3, 10):
        ...
        if nsweeps == 3 and (i == -3 or i == 9):
            assert np.mean(niters) <= 2, (
                "ERROR: too much iterations for advective asymptotics, got %s"
                % np.mean(niters)
            )
...
def plot_results(nsweeps):
    ...
    plt.xlim(
        min(xvalues_diff + xvalues_adv) / 10.0, max(xvalues_diff + xvalues_adv) * 10.0
    )
    ...
    # plot
    plt.semilogx(
        xvalues_diff, niter_diff, "r-", marker="s", markersize=10, label="diffusion"
    )
    plt.semilogx(
        xvalues_adv, niter_adv, "b-", marker="o", markersize=10, label="advection"
    )
