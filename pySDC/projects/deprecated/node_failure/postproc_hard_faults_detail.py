...
def create_plots(setup, cwd=""):
    ...
    setup_list = [
        ...
        (
            setup + "_steps_vs_iteration_hf_INTERP.npz",
            "INTERP",
            "2-sided",
            "orange",
            "o",
        ...
        (
            setup + "_steps_vs_iteration_hf_SPREAD_PREDICT.npz",
            "SPREAD_PREDICT",
            "1-sided + corr",
            "blue",
            "s",
        ...
        (
            setup + "_steps_vs_iteration_hf_INTERP_PREDICT.npz",
            "INTERP_PREDICT",
            "2-sided + corr",
            "green",
            "d",
    ...
    top = bottom + height
    rect = plt.Rectangle(
        xy=(left, bottom), width=width, height=height, color="lightgrey"
    )
    plt.text(
        0.5 * (left + right),
        0.5 * (bottom + top),
