    ...
    for qd_type in run_params["qd_list"]:
        ...
        for num_nodes in run_params["num_nodes_list"]:
            ...
            for j, dt in enumerate(run_params["dt_list"]):
                ...
                conv_data[qd_type][num_nodes]["error"][j] = np.linalg.norm(
                    [err[j][1] for j in range(len(err))], np.inf
                )
                conv_data[qd_type][num_nodes]["niter"][j] = round(
