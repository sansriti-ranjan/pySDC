...
def run(description, controller_params, run_params):
    ...
    for qd_type in run_params["qd_list"]:
        ...
        for num_nodes in run_params["num_nodes_list"]:
            ...
            conv_data[qd_type][num_nodes]["error"] = np.zeros_like(
                run_params["max_iter_list"], dtype=float
            )
            conv_data[qd_type][num_nodes]["residual"] = np.zeros_like(
                run_params["max_iter_list"], dtype=float
            )
            conv_data[qd_type][num_nodes]["niter"] = np.zeros_like(
                run_params["max_iter_list"], dtype="int"
            )
            conv_data[qd_type][num_nodes]["max_iter"] = run_params["max_iter_list"]

            for i, max_iter in enumerate(run_params["max_iter_list"]):
                print(
                    "Working on Qdelta=%s -- num. nodes=%i -- max. iter.=%i"
                    % (qd_type, num_nodes, max_iter)
                )
                ...
                uend, stats = controller.run(
                    u0=uinit, t0=run_params["t0"], Tend=run_params["tend"]
                )
                ...
                conv_data[qd_type][num_nodes]["error"][i] = np.linalg.norm(
                    [err[j][1] for j in range(len(err))], np.inf
                )
                conv_data[qd_type][num_nodes]["residual"][i] = np.linalg.norm(
                    [residual[j][1] for j in range(len(residual))], np.inf
                )
                conv_data[qd_type][num_nodes]["niter"][i] = round(
                    statistics.mean(niter.values())
                )
                print(
                    "Error=",
