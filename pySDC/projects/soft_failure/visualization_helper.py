...
def show_min_max_residual_across_simulation(
    fname, minres, maxres, meanres, medianres, maxiter
):
    """
    ...
    plt.plot(
        np.linspace(1, maxiter, num=maxiter), np.log10(minres), "ob--", label="min"
    )
    plt.plot(
        np.linspace(1, maxiter, num=maxiter), np.log10(maxres), "og--", label="max"
    )
    plt.plot(
        np.linspace(1, maxiter, num=maxiter), np.log10(meanres), "or--", label="mean"
    )
    plt.plot(
        np.linspace(1, maxiter, num=maxiter),
        np.log10(medianres),
        "oy--",
        label="median",
    )
...
def show_iter_hist(fname, maxiter, nruns):
    ...
    plt.hist(
        maxiter,
        bins=np.arange(min(maxiter), max(maxiter) + 2, 1),
        align="left",
        rwidth=0.9,
    )
