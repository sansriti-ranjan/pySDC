...
def compare_controllers(type=None, par=0.0, f=None):
    ...
    controller_mat = controller_matrix_nonMPI(
        num_procs=4, controller_params=controller_params, description=description
    )

    controller_nomat = controller_nonMPI(
        num_procs=4, controller_params=controller_params, description=description
    )
    ...
    assert diff < 2.3e-15, (
        "ERROR: difference between matrix-based and matrix-free result is too large, got %s"
        % diff
    )
