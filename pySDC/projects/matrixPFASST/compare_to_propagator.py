...
from pySDC.implementations.transfer_classes.TransferMesh_NoCoarse import (
    mesh_to_mesh as mesh_to_mesh_nocoarse,
)
from pySDC.projects.matrixPFASST.controller_matrix_nonMPI import (
    controller_matrix_nonMPI,
)
...
def scalar_equation_setup():
    ...
    ] = mesh_to_mesh_nocoarse  # pass spatial transfer class
...
def compare_controllers(type=None, par=0.0, f=None):
    ...
    controller = controller_matrix_nonMPI(
        num_procs=4, controller_params=controller_params, description=description
    )
    ...
    assert niters.count(niters[0]) == len(
        niters
    ), "ERROR: not all time-steps have the same number of iterations"
    niter = niters[0]
    ...
    assert err_mat_prop < 2.0e-14, (
        "ERROR: difference between matrix-based and propagator result is too large, got %s"
        % err_mat_prop
    )
