...
def findomega(stab_fh):
    ...
    func = (sympy.exp(-1j * omega) - stab_fh[0, 0]) * (
        sympy.exp(-1j * omega) - stab_fh[1, 1]
...
def compute_and_plot_dispersion(Nsamples=15, K=3):
    ...
    for i in range(0, np.size(k_vec)):
        Cs = (
            -1j * k_vec[i] * np.array([[0.0, c_speed], [c_speed, 0.0]], dtype="complex")
        )
        Uadv = (
            -1j * k_vec[i] * np.array([[U_speed, 0.0], [0.0, U_speed]], dtype="complex")
        )
        ...
        for k in range(0, K):
            Mat_sweep = Mat_sweep + np.linalg.matrix_power(LHSinv.dot(RHS), k).dot(
                LHSinv
            )
        ##
    ...
    plt.plot(
        k_vec,
        (U_speed + c_speed) + np.zeros(np.size(k_vec)),
        "--",
        color="k",
        linewidth=1.5,
        label="Exact",
    ...
    filename = (
        "data/phase-K" + str(K) + "-M" + str(sweeper_params["num_nodes"]) + ".png"
    )
    ...
    plt.xticks([0, 1, 2, 3], fontsize=fs)
    filename = (
        "data/ampfactor-K" + str(K) + "-M" + str(sweeper_params["num_nodes"]) + ".png"
    )
    plt.gcf().savefig(filename, bbox_inches="tight")
