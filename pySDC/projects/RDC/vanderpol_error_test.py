...
def compute_RDC_errors():
    ...
    for maxiter in maxiter_list:
        ...
        err = np.linalg.norm(uend_rdc - ref_sol, np.inf) / np.linalg.norm(
            ref_sol, np.inf
        )
        print(
            "Maxiter = %2i -- Error: %8.4e" % (controller_rdc.MS[0].params.maxiter, err)
        )
        results[maxiter] = err
...
def plot_RDC_results(cwd=""):
    ...
    assert (
        "maxiter_list" in results
    ), "ERROR: expecting the list of maxiters in the results dictionary"
