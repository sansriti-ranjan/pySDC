...
def run_RDC(cwd=""):
    ...
    controller_rdc = controller_nonMPI(
    ...
    print(
        "MLRDC     : Mean number of iterations: %6.3f -- Error: %8.4e"
        % (mean_niter, err)
    )
    results.append((err, mean_niter))

    controller_pfasst = controller_nonMPI(
        num_procs=10, controller_params=controller_params, description=description
    )
    ...
    err = np.linalg.norm(uend_pfasst - ref_sol, np.inf) / np.linalg.norm(
        ref_sol, np.inf
    )
    print(
        "PFASST(10): Mean number of iterations: %6.3f -- Error: %8.4e"
        % (mean_niter, err)
    )
    results.append((err, mean_niter))
