...
from pySDC.projects.parallelSDC.GeneralizedFisher_1D_FD_implicit_Jac import (
    generalized_fisher_jac,
)
from pySDC.projects.parallelSDC.linearized_implicit_fixed_parallel import (
    linearized_implicit_fixed_parallel,
)
from pySDC.projects.parallelSDC.linearized_implicit_fixed_parallel_prec import (
...
def main():
    ...
    for sweeper in sweeper_list:
        ...
        for dt in dt_list:
            ...
            # filter statistics
            error_pre = get_sorted(stats, type="error_pre_iteration", sortby="iter")[0][
                1
            ]

            error_post = get_sorted(stats, type="error_post_iteration", sortby="iter")[
                0
            ][1]
            ...
            print(
                "error and reduction rate at time %s: %6.4e -- %6.4e"
...
def plot_graphs(cwd=""):
    ...
    plt_helper.plt.loglog(
        dt_list, [dt * 2 for dt in dt_list], lw=0.5, ls="--", color="k", label="linear"
    )
    plt_helper.plt.loglog(
        dt_list,
        [dt * dt / dt_list[0] * 2 for dt in dt_list],
        lw=0.5,
        ls="-.",
        color="k",
        label="quadratic",
