...
def main(comm=None):
    ...
    for qd_type in qd_list:
        ...
        for setup, nvars, param_list in setup_list:
            ...
            if setup != "vanderpol":
                problem_params[
                    "nvars"
                ] = nvars  # number of degrees of freedom for each level
            ...
            for param in param_list:
                ...
                description[
                    "sweeper_params"
                ] = sweeper_params  # pass sweeper parameters
    ...
    assert (
        len(results) == (6 + 6 + 10 + 5) * 5 + 4
    ), "ERROR: did not get all results, got %s" % len(results)
    ...
    if comm.Get_rank() == 0:
        ...
        assert os.path.isfile(
            "data/parallelSDC_iterations_precond_MPI.pkl"
...
def plot_iterations():
    ...
    assert len(qd_type_list) == 5, (
        "ERROR did not find four preconditioners, got %s" % qd_type_list
    )
