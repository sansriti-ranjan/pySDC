...
def main():
    def rho(x):
        return max(
            abs(
                np.linalg.eigvals(
                    np.eye(M) - np.diag([x[i] for i in range(M)]).dot(coll.Qmat[1:, 1:])
                )
            )
        )
    ...
    for idx, x in enumerate(xdim):
        for idy, y in enumerate(ydim):
            minfield[idx, idy] = max(
                abs(
                    np.linalg.eigvals(
                        np.eye(M) - np.diag([x, y]).dot(coll.Qmat[1:, 1:])
                    )
                )
            )
    ...
    plt.text(
        d.x[0], d.x[1], "X", horizontalalignment="center", verticalalignment="center"
    )
    ...
    minfield_part = minfield[
        int(0.25 * numsteps) : int(0.75 * numsteps) + 1, 0 : int(0.25 * numsteps)
    ]
