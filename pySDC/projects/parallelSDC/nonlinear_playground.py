...
def main():
    ...
    results["xvalues"] = np.array(
        [(i + 1 - (P.nvars + 1) / 2) * P.dx for i in range(P.nvars)]
    )
    results["uinit"] = uinit
    results["uend"] = uend
    results["uex"] = uex
    ...
    assert os.path.isfile(
        "data/parallelSDC_results_graphs.pkl"
...
def plot_graphs():
    ...
    plt_helper.plt.plot(
        xvalues, uend, "bs", lw=1, markeredgecolor="k", label="computed"
    )
