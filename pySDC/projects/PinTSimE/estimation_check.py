...
def run(cwd="./"):
    ...
    for problem, sweeper in zip(problem_classes, sweeper_classes):
            ...
            for use_SE in use_switch_estimator:
                for use_A in use_adaptivity:
                    ...
                    stats = controller_run(
                        description, controller_params, use_A, use_SE, t0, Tend
                    )
                    ...
                    if use_SE:
                        assert (
                            len(get_recomputed(stats, type="switch", sortby="time"))
                            >= 1
                        ), "No switches found for dt={}!".format(dt_item)

                    fname = "data/battery_dt{}_USE{}_USA{}_{}.dat".format(
                        dt_item, use_SE, use_A, sweeper.__name__
                    )
                    f = open(fname, "wb")
                    dill.dump(stats, f)
                    f.close()
                    ...
                    if use_SE or use_A:
                        restarts_sorted = np.array(
                            get_sorted(stats, type="restart", recomputed=None)
                        )[:, 1]
        ...
        iterations_over_time(
            dt_list,
            description["step_params"]["maxiter"],
            problem.__name__,
            sweeper.__name__,
        )
...
def accuracy_check(dt_list, problem, sweeper, V_ref, cwd="./"):
    ...
    for dt_item in dt_list:
        f3 = open(
            cwd + "data/battery_dt{}_USETrue_USATrue_{}.dat".format(dt_item, sweeper),
            "rb",
        )
        ...
        f4 = open(
            cwd + "data/battery_dt{}_USEFalse_USATrue_{}.dat".format(dt_item, sweeper),
            "rb",
        )
        ...
        e_emb_SE_adapt_val = get_sorted(
            stats_SE_adapt, type="e_embedded", recomputed=False
        )
        ...
        if len(dt_list) > 1:
            ...
            e_plt1 = e_ax.plot(
                times_SE_adapt, e_emb_SE_adapt, "k--", label=r"SE+A - $\epsilon_{emb}$"
            )
            e_plt2 = e_ax.plot(
                times_adapt, e_emb_adapt, "g--", label=r"A - $\epsilon_{emb}$"
            )
            ...
            ax_acc[count_ax].legend(
                lines, labels, frameon=False, fontsize=6, loc="upper right"
            )
    ...
    fig_acc.savefig(
        "data/embedded_error_adaptivity_{}.png".format(sweeper),
        dpi=300,
        bbox_inches="tight",
    )
...
def differences_around_switch(
    ...
    for dt_item in dt_list:
        f1 = open(
            cwd + "data/battery_dt{}_USETrue_USAFalse_{}.dat".format(dt_item, sweeper),
            "rb",
        )
        ...
        f2 = open(
            cwd + "data/battery_dt{}_USEFalse_USAFalse_{}.dat".format(dt_item, sweeper),
            "rb",
        )
        stats = dill.load(f2)
        f2.close()
        ...
        vC_adapt = [
            me[1][1] for me in get_sorted(stats_adapt, type="u", recomputed=False)
        ]
        vC_SE_adapt = [
            me[1][1] for me in get_sorted(stats_SE_adapt, type="u", recomputed=False)
        ]
        ...
        times_adapt = [
            me[0] for me in get_sorted(stats_adapt, type="u", recomputed=False)
        ]
        times_SE_adapt = [
            me[0] for me in get_sorted(stats_SE_adapt, type="u", recomputed=False)
        ]
        ...
        diffs_true_at.append(
            [
                ...
                if np.isclose(times_SE[m], t_switch, atol=1e-15)
            ][0]
        )
    ...
    setup_mpl()
    fig_around, ax_around = plt_helper.plt.subplots(
        1, 3, figsize=(9, 3), sharex="col", sharey="row"
    )
    ...
    pos21 = ax_around[1].plot(
        dt_list, diffs_false_before_adapt, "rs-", label="before switch"
    )
    pos22 = ax_around[1].plot(
        dt_list, diffs_false_after_adapt, "bd--", label="after switch"
    )
    ...
    restarts_plt2 = restart_ax2.plot(
        dt_list, restarts_SE_adapt, "cs--", label="Restarts"
    )
    ...
    fig_around.savefig(
        "data/diffs_around_switch_{}.png".format(sweeper), dpi=300, bbox_inches="tight"
    )
...
def differences_over_time(dt_list, problem, sweeper, V_ref, cwd="./"):
    ...
    for dt_item in dt_list:
        ...
        restart_adapt = np.array(
            get_sorted(stats_adapt, type="restart", recomputed=None)
        )
        restart_SE_adapt = np.array(
            get_sorted(stats_SE_adapt, type="restart", recomputed=None)
        )
        ...
        if len(dt_list) > 1:
            ...
            ax_diffs[0, count_ax].plot(
                times_SE, diff_SE, label="SE=True, A=False", color="#ff7f0e"
            )
            ax_diffs[0, count_ax].plot(
                times, diff, label="SE=False, A=False", color="#1f77b4"
            )
            ax_diffs[0, count_ax].plot(
                times_adapt,
                diff_adapt,
                label="SE=False, A=True",
                color="red",
                linestyle="--",
            )
            ax_diffs[0, count_ax].plot(
                times_SE_adapt,
                diff_SE_adapt,
                label="SE=True, A=True",
                color="limegreen",
                linestyle="-.",
            )
            ax_diffs[0, count_ax].axvline(
                x=t_switch_SE, linestyle="--", linewidth=0.5, color="k", label="Switch"
            )
            ...
            if count_ax == 0:
                ax_diffs[0, count_ax].set_ylabel(
                    "Difference $v_{C}-V_{ref}$", fontsize=6
                )

            if count_ax == 0 or count_ax == 1:
                ax_diffs[0, count_ax].legend(
            ...
            for i in range(len(restart_adapt)):
                if restart_adapt[i, 1] > 0:
                    ax_diffs[1, count_ax].axvline(
                        restart_adapt[i, 0], color="grey", linestyle="-."
                    )
    ...
    fig_diffs.savefig(
        "data/diffs_over_time_{}.png".format(sweeper), dpi=300, bbox_inches="tight"
    )
...
def iterations_over_time(dt_list, maxiter, problem, sweeper, cwd="./"):
    ...
    if len(dt_list) > 1:
        ...
        for col in range(len(dt_list)):
            ...
            ax_iter_all[col].plot(
                times_SE_adapt[col], iters_time_SE_adapt[col], "--", label="SE=T, A=T"
            )
            ax_iter_all[col].plot(
                times_adapt[col], iters_time_adapt[col], "--", label="SE=F, A=T"
            )
        ...
        ax_iter_all.plot(
            times_SE_adapt[0], iters_time_SE_adapt[0], "--", label="SE=T, A=T"
        )
        ax_iter_all.plot(times_adapt[0], iters_time_adapt[0], "--", label="SE=F, A=T")
        ax_iter_all.axvline(
            x=t_switches_SE[0], linestyle="--", linewidth=0.5, color="k", label="Switch"
        )
    ...
    plt_helper.plt.tight_layout()
    fig_iter_all.savefig(
        "data/iters_{}.png".format(sweeper), dpi=300, bbox_inches="tight"
    )
