...
def controller_run(
    description, controller_params, use_adaptivity, use_switch_estimator, t0, Tend
):
    """
...
def plot_voltages(
    ...
    if use_switch_estimator:
        ...
        ax.axvline(
            x=t_switch[-1], linestyle="--", linewidth=0.8, color="r", label="Switch"
        )
    ...
    fig.savefig(
        "data/{}_model_solution_{}.png".format(problem, sweeper),
        dpi=300,
        bbox_inches="tight",
    )
...
def get_data_dict(stats, use_adaptivity, use_switch_estimator, recomputed=False):
    ...
    data["cL"] = np.array(
        [
            me[1][0]
            for me in get_sorted(stats, type="u", recomputed=recomputed, sortby="time")
        ]
    )
    data["vC"] = np.array(
        [
    ...
    if use_adaptivity:
        data["dt"] = np.array(
            get_sorted(stats, type="dt", recomputed=recomputed, sortby="time")
        )[:, 1]
        data["e_em"] = np.array(
            get_sorted(
                stats,
    ...
    if use_switch_estimator:
        data["switches"] = np.array(
            get_recomputed(stats, type="switch", sortby="time")
        )[:, 1]
    ...
    filtered_list = [
        filter_stats(
            ...
            num_restarts=max(
                [
                    me.num_restarts
                    for me in filter_stats(stats, type=type, time=t_unique).keys()
                ]
            ),
            type=type,
        )
...
def proof_assertions_description(description, use_adaptivity, use_switch_estimator):
    ...
    assert (
        description["problem_params"]["alpha"]
        > description["problem_params"]["V_ref"][k]
        for k in range(n)
    ), 'Please set "alpha" greater than values of "V_ref"'
    assert (
        type(description["problem_params"]["V_ref"]) == np.ndarray
    ), '"V_ref" needs to be an np.ndarray'
    assert (
        type(description["problem_params"]["C"]) == np.ndarray
    ), '"C" needs to be an np.ndarray '
    assert (
