...
def run(cwd="./"):
        ...
        for dt_item in dt_list:
            for use_SE in use_switch_estimator:
                ...
                if use_SE:
                    ...
                    assert (
                        len(switches) >= 2
                    ), f"Expected at least 2 switches for dt: {dt_item}, got {len(switches)}!"

                    check_solution(stats, dt_item, use_SE)

                fname = "data/{}_dt{}_USE{}.dat".format(
                    problem.__name__, dt_item, use_SE
                )
                f = open(fname, "wb")
                dill.dump(stats, f)
                f.close()
                ...
                if use_SE:
                    restarts_dict[dt_item] = np.array(
    ...
    for dt_item in dt_list:
        f1 = open(
            cwd + "data/{}_dt{}_USETrue.dat".format(problem.__name__, dt_item), "rb"
        )
        stats_true = dill.load(f1)
        f1.close()

        f2 = open(
            cwd + "data/{}_dt{}_USEFalse.dat".format(problem.__name__, dt_item), "rb"
        )
        ...
        vC1_true = [
            me[1][1] for me in get_sorted(stats_true, type="u", recomputed=False)
        ]
        vC2_true = [
            me[1][2] for me in get_sorted(stats_true, type="u", recomputed=False)
        ]
        vC1_false = [
            me[1][1] for me in get_sorted(stats_false, type="u", recomputed=False)
        ]
        vC2_false = [
            me[1][2] for me in get_sorted(stats_false, type="u", recomputed=False)
        ]
        ...
        diff_true_all1.append(
            [
                diff_true1[m]
                for m in range(len(t_true))
                if np.isclose(t_true[m], t_switch[0], atol=1e-15)
            ]
        )
        diff_true_all2.append(
            [
                diff_true2[m]
                for m in range(len(t_true))
                if np.isclose(t_true[m], t_switch[1], atol=1e-15)
            ]
        )
        ...
        diff_false_all_before1.append(
            [
                diff_false1[m - 1]
                for m in range(1, len(t_false))
                if t_false[m - 1] < t_switch[0] < t_false[m]
            ]
        )
        ...
        diff_false_all_before2.append(
            [
                diff_false2[m - 1]
                for m in range(1, len(t_false))
                if t_false[m - 1] < t_switch[1] < t_false[m]
            ]
        )
        ...
        fig1.savefig(
            "data/difference_estimation_vC1_dt{}.png".format(dt_item),
            dpi=300,
            bbox_inches="tight",
        )
        ...
        fig2.savefig(
            "data/difference_estimation_vC2_dt{}.png".format(dt_item),
            dpi=300,
            bbox_inches="tight",
        )
    ...
    pos1 = ax1.plot(
        dt_list, diff_false_all_before1, "rs-", label="SE=False - before switch1"
    )
    pos2 = ax1.plot(
        dt_list, diff_false_all_after1, "bd-", label="SE=False - after switch1"
    )
    ...
    pos1 = ax2.plot(
        dt_list, diff_false_all_before2, "rs-", label="SE=False - before switch2"
    )
    pos2 = ax2.plot(
        dt_list, diff_false_all_after2, "bd-", label="SE=False - after switch2"
    )
