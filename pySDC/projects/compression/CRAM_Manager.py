...
class CRAM_Manager:
    ...
    #numVectors is M
    def registerVar(self, varName, shape, dtype=np.float64, numVectors=100, errBoundMode="ABS", compType="sz", errBound=1e-5):
        if varName not in self.mem_map:
            ...
            self.mem_map[varName] = (compressor, [compressor.encode(np.ones(shape)) for i in range(numVectors)], shape) # TODO finish
...
if __name__=="__main__":
    ...
    memory.registerVar("cat", arr.shape, dtype=np.float64, numVectors=1, errBoundMode="ABS", compType="sz", errBound=0.1)
