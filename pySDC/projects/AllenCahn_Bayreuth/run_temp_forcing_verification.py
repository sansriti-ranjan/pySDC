...
from pySDC.implementations.problem_classes.AllenCahn_Temp_MPIFFT import (
    allencahn_temp_imex,
)
...
def run_simulation(
    name="", spectral=None, nprocs_time=None, nprocs_space=None, dt=None, cwd="."
):
    """
    ...
    assert (
        world_size == space_size
    ), "This script cannot run parallel-in-time with MPI, only spatial parallelism"
    ...
    if space_rank == 0:
        ...
        if spectral:
            ...
            err = max(
                np.amax(abs(ureal - array[..., 0])), np.amax(abs(Treal - array[..., 1]))
            )
        else:
            err = abs(array - uend)
...
def main(nprocs_space=None, cwd="."):
    ...
    assert np.isclose(
        orders[3], 5.3, rtol=2e-02
    ), f"Order of accuracy is not within tolerance, got {orders[3]}"
