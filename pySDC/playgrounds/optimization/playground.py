...
def rho(x):
    global Q, M
    return np.linalg.norm(Q - np.diag([x[i] for i in range(M)]), np.inf) / np.amin(
        [abs(x[i]) for i in range(M)]
    )
...
for prec in prec_list:
    ...
    print(
        ...
        np.linalg.norm(
            np.linalg.matrix_power(
                A.dot(np.eye(M) - np.linalg.inv(QDelta).dot(Q)).dot(B), 9
            ),
            np.inf,
        ),
    )
    ...
    for i, ldt in enumerate(ldt_list):
        R = np.linalg.matrix_power(
            ldt * np.linalg.inv(np.eye(M) - ldt * QDelta).dot(Q - QDelta), 1
        )
        ...
        result_est[i] = np.linalg.norm(
            tmp.dot(np.linalg.inv(np.eye(M) - ldt * QDelta).dot(H)), np.inf
        )
