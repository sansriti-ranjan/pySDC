...
def compare_preconditioners(f=None, list_of_k=None):
    ...
    for k in list_of_k:
        description_IE, controller_params_IE = testequation_setup(
            prec_type="MIN3", maxiter=k
        )
        description_LU, controller_params_LU = testequation_setup(
            prec_type="LU", maxiter=k
        )
        ...
        controller_IE = controller_nonMPI(
            num_procs=1,
            controller_params=controller_params_IE,
            description=description_IE,
        )
        controller_LU = controller_nonMPI(
            num_procs=1,
            controller_params=controller_params_LU,
