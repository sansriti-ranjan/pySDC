...
def get_alpha(m0, rhs1, uL, Lip, L):
    ...
    m0 = (
        (alpha + L * Lip) / (1 - alpha) * m0
        + L * 3 * eps * norm_rhs1 / alpha
        + L * 3 * eps * abs(uL)
    )
...
def run_paralpha(problem, time_params, err_euler):
    ...
    Lip = (
        problem.params.c
        * dt
        * max(abs(np.linalg.eigvals(problem.A.todense())))
        / problem.dx
    )
    ...
    while k < kmax and err > err_euler * 5:
        ...
        print(
            f"Diagonalization error: {np.linalg.norm(S @ np.diag(d) @ Sinv - Ea, np.inf)}"
        )
