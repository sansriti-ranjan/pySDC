...
class allencahn_imex(ptype):
    ...
    def eval_f(self, u, t):
        ...
        if self.params.eps > 0:
            f.expl.values[..., 0] = -2.0 / self.params.eps**2 * u.values[..., 0] * (
                1.0 - u.values[..., 0]
            ) * (1.0 - 2.0 * u.values[..., 0]) - 6.0 * self.params.dw * (
                u.values[..., 1] - self.params.TM
            ) / self.params.TM * u.values[
                ..., 0
            ] * (
                1.0 - u.values[..., 0]
            )
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        me.values[..., 1] = (
            tmp_rhs.r2c()
            .apply(linear_solve_param, out=Ellipsis)
            .c2r(out=Ellipsis)
            .value
        )
        return me
