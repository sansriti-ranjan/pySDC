...
class dynamogp_2d_dedalus(ptype):
    ...
    def eval_f(self, u, t):
        ...
        f.expl.values[0] = (
            -self.u * u.values[0] * 1j * self.params.kx
            - self.v * by_y
            - self.w * by_z
            + u.values[1] * self.v_z
        ).evaluate()
        f.expl.values[1] = (
            -self.u * u.values[1] * 1j * self.params.kx
            - self.v * bz_y
            - self.w * bz_z
            + u.values[0] * self.w_y
        ).evaluate()
        ...
        f.impl.values[0] = (
            ...
            / self.params.Rm
            * (
                u.values[0].differentiate(z=2)
                + u.values[0].differentiate(y=2)
                - self.params.kx**2 * u.values[0]
            )
        ).evaluate()
        f.impl.values[1] = (
            1.0
            / self.params.Rm
            * (
                u.values[1].differentiate(z=2)
                + u.values[1].differentiate(y=2)
                - self.params.kx**2 * u.values[1]
            )
        ).evaluate()
