...
class dynamo_2d_dedalus(ptype):
    ...
    def __init__(
        ...
        xbasis = de.Fourier(
            "x", problem_params["nvars"][0], interval=(0, 2 * np.pi), dealias=1
        )
        ybasis = de.Fourier(
            "y", problem_params["nvars"][1], interval=(0, 2 * np.pi), dealias=1
        )
        domain = de.Domain(
            [xbasis, ybasis], grid_dtype=np.complex128, comm=problem_params["comm"]
        )
        ...
        self.problem_imp.add_equation(
            "dt(b_x) - (1/Rm) * ( dx(dx(b_x)) + dy(dy(b_x)) - kz ** 2 * (b_x) ) = 0"
        )
        self.problem_imp.add_equation(
            "dt(b_y) - (1/Rm) * ( dx(dx(b_y)) + dy(dy(b_y)) - kz ** 2 * (b_y) ) = 0"
        )
        ...
        self.problem_exp.add_equation(
            "dt(b_x) = -u_x * dx(b_x) - u_y * dy(b_x) - u_z*1j*kz *b_x  + b_y * dy(u_x)"
        )
        self.problem_exp.add_equation(
            "dt(b_y) = -u_x * dx(b_y) - u_y * dy(b_y) - u_z*1j*kz *b_y  + b_x * dx(u_y)"
        )
    ...
    def u_exact(self, t):
        ...
        if self.params.initial == "random":
            me.values[0]["g"] = np.random.uniform(
                low=-1e-5, high=1e-5, size=(xvar_loc, yvar_loc)
            )
            me.values[1]["g"] = np.random.uniform(
                low=-1e-5, high=1e-5, size=(xvar_loc, yvar_loc)
            )
