...
def get2DMesh(N, x_b, z_b, bc_hor, bc_ver):
    assert (
        np.size(N) == 2
    ), "N needs to be an array with two entries: N[0]=Nx and N[1]=Nz"
    assert (
        np.size(x_b) == 2
    ...
    if bc_hor[0] in ["periodic"]:
        assert bc_hor[1] in [
            "periodic"
        ], "Periodic boundary conditions must be prescribed at both boundaries"
...
def get2DMatrix(N, h, bc_hor, bc_ver):
    ...
    assert (
        np.size(h) == 2
    ), "h needs to be an array with two entries: h[0]=dx and h[1]=dz"
