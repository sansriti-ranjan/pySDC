...
def plot_data():
    ...
    speedup_overres = [
        p
        / (
            p / niter_overres[0] * alpha_overres
            + k / niter_overres[0] * (1 + alpha_overres)
        )
    ...
    speedup_wellres_1 = [
        p
        / (
            p / niter_wellres_1[0] * alpha_wellres_1
            + k / niter_wellres_1[0] * (1 + alpha_wellres_1)
        )
    ...
    speedup_wellres_2 = [
        p
        / (
            p / niter_wellres_2[0] * alpha_wellres_2
            + k / niter_wellres_2[0] * (1 + alpha_wellres_2)
        )
    ...
    plt_helper.plt.semilogx(
        ...
        marker="d",
        markersize=6,
        label=r"$Nx_\mathcal{F}=32, \alpha=\frac{1}{4}$",
    )
    plt_helper.plt.semilogx(
        nprocs,
        speedup_wellres_2,
        ...
        marker="s",
        markersize=6,
        label=r"$Nx_\mathcal{F}=32, \alpha=\frac{1}{2}$",
    )
