...
def plot_data():
    ...
    speedup_fft_overres = [
        p / (i / niter_fft_overres[0]) for p, i in zip(nprocs, niter_fft_overres)
    ]
    speedup_fft = [p / (i / niter_fft_overres[0]) for p, i in zip(nprocs, niter_fft)]
    speedup_fft_slight = [
        p / (i / niter_fft_overres[0]) for p, i in zip(nprocs, niter_fft_slight)
    ]

    plt_helper.setup_mpl()
    plt_helper.newfig(textwidth=238.96, scale=1.0)
    plt_helper.plt.semilogx(
        nprocs, speedup_fd, color="b", marker="o", markersize=6, label="FD, Nx=128"
    )
    ...
    plt_helper.plt.semilogx(
        nprocs, speedup_fft, color="r", marker="v", markersize=6, label="Spectral, Nx=8"
    )
