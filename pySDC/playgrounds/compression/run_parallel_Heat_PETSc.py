...
def main(nprocs_space=None):
    ...
    if space_rank == 0:
        ...
        out = f"   Time-rank {time_rank} -- Mean number of iterations: %4.2f" % np.mean(
            niters
        )
        print(out)
        out = (
            f"   Time-rank {time_rank} -- Range of values for number of iterations: %2i "
            % np.ptp(niters)
        )
        print(out)
        out = (
            f"   Time-rank {time_rank} -- Position of max/min number of iterations: %2i -- %2i"
            % (
        ...
        print(out)
        out = (
            f"   Time-rank {time_rank} -- Std and var for number of iterations: %4.2f -- %4.2f"
            % (
                float(np.std(niters)),
                float(np.var(niters)),
            )
