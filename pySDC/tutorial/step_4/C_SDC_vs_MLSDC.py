...
def main():
    ...
    description_mlsdc[
        "problem_params"
    ] = problem_params_mlsdc  # pass problem parameters
    description_mlsdc["sweeper_class"] = generic_LU  # pass sweeper (see part B)
    description_mlsdc[
        "sweeper_params"
    ] = sweeper_params_mlsdc  # pass sweeper parameters
    ...
    assert diff < 6e-10, (
        "ERROR: difference between MLSDC and SDC is higher than expected, got %s" % diff
    )
    assert niter_sdc - niter_mlsdc <= 6, (
        "ERROR: MLSDC required more iterations than expected, got %s" % niter_mlsdc
    )
