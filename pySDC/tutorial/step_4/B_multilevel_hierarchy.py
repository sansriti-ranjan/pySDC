...
def main():
    ...
    for l in range(len(S.levels)):
        ...
        out = "Level %2i: nvars = %4i -- nnodes = %2i" % (
            l,
            L.prob.nvars[0],
            L.sweep.coll.num_nodes,
        )
        f.write(out + "\n")
        print(out)
        assert (
            L.prob.nvars[0]
            == problem_params["nvars"][min(l, len(problem_params["nvars"]) - 1)]
        ), ("ERROR: number of DOFs is not correct on this level, got %s" % L.prob.nvars)
        assert (
            L.sweep.coll.num_nodes
            == sweeper_params["num_nodes"][min(l, len(sweeper_params["num_nodes"]) - 1)]
        ), (
            "ERROR: number of nodes is not correct on this level, got %s"
            % L.sweep.coll.num_nodes
        )
    f.close()
