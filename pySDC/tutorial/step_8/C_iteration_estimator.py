...
def setup_diffusion(dt=None, ndim=None, ml=False):
    ...
    if ml:
        ...
        ] = mesh_to_mesh  # pass spatial transfer class
        description[
            "space_transfer_params"
        ] = space_transfer_params  # pass parameters for spatial transfer
...
def setup_auzinger(dt=None, ml=False):
    ...
    if ml:
        description[
            "space_transfer_class"
        ] = mesh_to_mesh_nc  # pass spatial transfer class

    return description, controller_params


def run_simulations(
    type=None, ndim_list=None, Tend=None, nsteps_list=None, ml=False, nprocs=None
):
    """
    ...
    for ndim in ndim_list:
        for nsteps in nsteps_list:
            ...
            for iters, PDE_err, coll_err in zip(iter_counts, PDE_errors, coll_errors):
                assert (
                    coll_err[1] < description["step_params"]["errtol"]
                ), f"Error too high, got {coll_err[1]:8.4e}"
                out = (
...
def main():
    run_simulations(
        type="diffusion", ndim_list=[1], Tend=1.0, nsteps_list=[8], ml=False, nprocs=1
    )
    run_simulations(
        type="diffusion", ndim_list=[1], Tend=1.0, nsteps_list=[8], ml=True, nprocs=1
    )

    run_simulations(
        type="advection", ndim_list=[1], Tend=1.0, nsteps_list=[8], ml=False, nprocs=1
    )
    run_simulations(
        type="advection", ndim_list=[1], Tend=1.0, nsteps_list=[8], ml=True, nprocs=1
    )

    run_simulations(
        type="auzinger", ndim_list=[1], Tend=1.0, nsteps_list=[8], ml=False, nprocs=1
    )
    run_simulations(
        type="auzinger", ndim_list=[1], Tend=1.0, nsteps_list=[8], ml=True, nprocs=1
    )
