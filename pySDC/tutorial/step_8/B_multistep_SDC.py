...
def main():
    ...
    uend_mssdc_jac, stats_mssdc_jac = controller_mssdc_jac.run(
        u0=uinit, t0=t0, Tend=Tend
    )
    ...
    show_residual_across_simulation(
        stats_mssdc_jac, "data/step_8_residuals_mssdc_jac.png"
    )
    show_residual_across_simulation(
        stats_mssdc_gs, "data/step_8_residuals_mssdc_gs.png"
    )
    ...
    assert diff_jac < 3.1e-10, (
        "ERROR: difference between PFASST and parallel MSSDC controller is too large, got %s"
        % diff_jac
    )
    assert diff_gs < 3.1e-10, (
        "ERROR: difference between PFASST and serial MSSDC controller is too large, got %s"
        % diff_gs
    )
    assert diff_jac_gs < 3.1e-10, (
        "ERROR: difference between parallel and serial MSSDC controller is too large, got %s"
        % diff_jac_gs
    )
