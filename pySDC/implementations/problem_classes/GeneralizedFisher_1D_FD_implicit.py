...
class generalized_fisher(ptype):
    ...
    def __init__(
        self, nvars, nu, lambda0, newton_maxiter, newton_tol, interval, stop_at_nan=True
    ):
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        ul = (
            1
            + (2 ** (nu / 2.0) - 1)
            * np.exp(-nu / 2.0 * sig1 * (self.interval[0] + 2 * lam1 * t))
        ) ** (-2.0 / nu)
        ur = (
            1
            + (2 ** (nu / 2.0) - 1)
            * np.exp(-nu / 2.0 * sig1 * (self.interval[1] + 2 * lam1 * t))
        ) ** (-2.0 / nu)
        ...
        while n < self.newton_maxiter:
            ...
            g = (
                u
                - factor * (self.A.dot(uext)[1:-1] + lambda0**2 * u * (1 - u**nu))
                - rhs
            )
            ...
            dg = sp.eye(self.nvars) - factor * (
                self.A[1:-1, 1:-1]
                + sp.diags(lambda0**2 - lambda0**2 * (nu + 1) * u**nu, offsets=0)
            )
    ...
    def u_exact(self, t):
        ...
        me[:] = (
            1
            + (2 ** (self.nu / 2.0) - 1)
            * np.exp(-self.nu / 2.0 * sig1 * (xvalues + 2 * lam1 * t))
        ) ** (-2.0 / self.nu)
        return me
