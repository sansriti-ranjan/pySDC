...
class allencahn2d_imex(ptype):  # pragma: no cover
    ...
    def __init__(self, nvars, nu, eps, radius, L=1.0, init_type="circle"):
        ...
        self.xvalues = cp.array(
            [i * self.dx - self.L / 2.0 for i in range(self.nvars[0])]
        )
        ...
        kx[: int(self.init[0][0] / 2) + 1] = (
            2 * np.pi / self.L * cp.arange(0, int(self.init[0][0] / 2) + 1)
        )
        kx[int(self.init[0][0] / 2) + 1 :] = (
            2
            * np.pi
    ...
    def eval_f(self, u, t):
        ...
        if self.eps > 0:
            f.expl[:] = (1.0 / self.eps**2 * v * (1.0 - v**self.nu)).reshape(
                self.nvars
            )
        return f
    ...
    def u_exact(self, t):
        ...
        if self.init_type == "circle":
            ...
            me[:, :] = cp.tanh(
                (self.radius - cp.sqrt(xv**2 + yv**2)) / (cp.sqrt(2) * self.eps)
            )
