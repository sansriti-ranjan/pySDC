...
class allencahn_fullyimplicit(ptype):  # pragma: no cover
    ...
    def __init__(
        self, nvars, nu, eps, newton_maxiter, newton_tol, lin_tol, lin_maxiter, radius
    ):
        """
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        while n < self.newton_maxiter:
            ...
            # assemble dg
            dg = Id - factor * (
                self.A + 1.0 / eps2 * csp.diags((1.0 - (nu + 1) * u**nu), offsets=0)
            )
    ...
    def u_exact(self, t):
        ...
        me[:] = cp.tanh(
            (self.radius - cp.sqrt(mx**2 + my**2)) / (cp.sqrt(2) * self.eps)
        )
        # print(type(me))
        return me
...
class allencahn_semiimplicit_v2(allencahn_fullyimplicit):
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        while n < self.newton_maxiter:
            ...
            # assemble dg
            dg = Id - factor * (
                self.A - 1.0 / eps2 * csp.diags(((nu + 1) * u**nu), offsets=0)
            )
...
class allencahn_multiimplicit(allencahn_fullyimplicit):
    ...
    def solve_system_2(self, rhs, factor, u0, t):
        ...
        while n < self.newton_maxiter:
            ...
            # assemble dg
            dg = Id - factor * (
                1.0 / eps2 * csp.diags((1.0 - (nu + 1) * u**nu), offsets=0)
            )
