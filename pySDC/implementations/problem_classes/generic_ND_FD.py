...
class GenericNDimFinDiff(ptype):
    ...
    def __init__(
        ...
        super().__init__(
            init=(nvars[0] if ndim == 1 else nvars, None, np.dtype("float64"))
        )
        ...
        self._makeAttributeAndRegister(
            "nvars", "stencil_type", "order", "bc", localVars=locals(), readOnly=True
        )
        self._makeAttributeAndRegister(
            "freq", "lintol", "liniter", "solver_type", localVars=locals()
        )
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        if solver_type == "direct":
            ...
            raise ValueError(
                f'solver type "{solver_type}" not known in generic advection-diffusion implementation!'
            )
