...
class allencahn_front_fullyimplicit(ptype):
    ...
    def __init__(
        self, nvars, dw, eps, newton_maxiter, newton_tol, interval, stop_at_nan=True
    ):
        ...
        self.xvalues = np.array(
            [(i + 1 - (self.nvars + 1) / 2) * self.dx for i in range(self.nvars)]
        )
        ...
        self.uext = self.dtype_u(
            (self.init[0] + 2, self.init[1], self.init[2]), val=0.0
        )
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        self.uext[0] = 0.5 * (
            1 + np.tanh((self.interval[0] - v * t) / (np.sqrt(2) * self.eps))
        )
        self.uext[-1] = 0.5 * (
            1 + np.tanh((self.interval[1] - v * t) / (np.sqrt(2) * self.eps))
        )
...
class allencahn_front_semiimplicit(allencahn_front_fullyimplicit):
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        self.uext[1:-1] = rhs[:]
        me[:] = spsolve(
            sp.eye(self.nvars + 2, format="csc") - factor * self.A, self.uext
        )[1:-1]
        return me
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        while n < self.newton_maxiter:
            ...
            g = (
                ...
                - rhs
                - factor
                * (self.A.dot(self.uext)[1:-1] - 1.0 * gprim - 6.0 * dw * u * (1.0 - u))
            )
            ...
            dgprim = (
                ...
                * (
                    ...
                    * 4
                    * a2
                    / (1.0 - a2 * (2.0 * u - 1.0) ** 2) ** 2
                )
            )

            dg = Id - factor * (
                A
                - 1.0 * sp.diags(dgprim, offsets=0)
                - 6.0 * dw * sp.diags((1.0 - u) - u, offsets=0)
            )
...
class allencahn_periodic_fullyimplicit(ptype):
    ...
    def __init__(
        ...
        self.xvalues = np.array(
            [self.interval[0] + i * self.dx for i in range(self.nvars)]
        )
    ...
    def u_exact(self, t):
        ...
        me[:] = 0.5 * (
            1
            + np.tanh(
                (self.radius - abs(self.xvalues) - v * t) / (np.sqrt(2) * self.eps)
            )
        )
        return me
...
class allencahn_periodic_semiimplicit(allencahn_periodic_fullyimplicit):
    ...
    def __init__(
        ...
        super().__init__(
            nvars, dw, eps, newton_maxiter, newton_tol, interval, radius, stop_at_nan
        )
