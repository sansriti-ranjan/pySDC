...
class BasicRestarting(ConvergenceController):
    ...
    def get_implementation(cls, flavor):
        ...
        if flavor == "MPI":
            ...
            raise NotImplementedError(
                f"Flavor {flavor} of BasicRestarting is not implemented!"
            )
    ...
    def setup(self, controller, params, description, **kwargs):
        ...
        defaults = {
            ...
            "step_size_spreader": SpreadStepSizesBlockwise.get_implementation(
                useMPI=params["useMPI"]
            ),
        }
            ...
            if self.buffers.max_restart_reached and S.status.restart:
                if self.params.crash_after_max_restarts:
                    raise ConvergenceError(
                        f"Restarted {S.status.restarts_in_a_row} time(s) already, surrendering now."
                    )
                self.log(
...
class BasicRestartingMPI(BasicRestarting):
    ...
    def __init__(self, controller, params, description, **kwargs):
        ...
        super().__init__(controller, params, description)
        self.buffers = Pars(
            {"restart": False, "max_restart_reached": False, "restart_earlier": False}
        )
...
on...",
        ...
        S.status.restart = (
            S.status.restart or self.buffers.restart_earlier
        ) and not self.buffers.max_restart_reached
        ...
        if not S.status.last:
            self.send(
                comm,
                dest=S.status.slot + 1,
                data=(S.status.restart, self.buffers.max_restart_reached),
            )
